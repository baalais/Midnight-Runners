...
const CartModal: React.FC<CartModalProps> = ({ onClose }) => {
  ...
  console.log("Wix Client in CartModal:", wixClient); // Debugging line

  const handleCheckout = async () => {
    try {
      if (!wixClient || !wixClient.currentCart) {
        console.error("WixClient or currentCart module is not available");
        return;
      }
      ...
      {!cart.lineItems ? (
        ...
        <>
                ...
                <div className="flex flex-col justify-between w-full">
                  <div>
                    <div className="flex items-center justify-between gap-8">
                      <h3 className="font-semibold text-black">{item.productName?.original}</h3>
          ...
          <div className="">
            ...
            <div className="flex justify-between text-sm">
              <button className="rounded-md py-3 px-4 ring-1 ring-gray-300 text-black">View Cart</button>
              <button
